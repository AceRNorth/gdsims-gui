    ...
    def closeEvent(self, event):
        ...
        if simRunning:  # gives specific warning message if sim is running
            reply = QMessageBox.question(self, "Warning",
                                         "The simulation is still running.\nAre you sure you want to abort the run and quit?",
                                         QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        else:
            # creates a message box with a question that the user needs to answer
            # first string appears in titlebar, second string is message displayed by the dialog
            # give message in the box and what combination of buttons appear in the dialog, and what the default button is
